...
`Align Before Action` 是一种先把想法聊清楚、再开始做的对齐方式。它适合想法还在形成中、上下文不完整，或者你不想让 AI 先猜再做的场景。它不只适合产品和开发，也适合日常决定、写作、表达、请求，或者任何还没想清楚的模糊想法。用户可以直接调用它；当普通输入里还有重要未决信息时，AI 也可以先用一句话建议是否进入这个 Skill。进入后，它会先弄清楚你到底想做什么，再一步一步补齐关键缺口，等双方理解一致后才执行。
...
- 用户明确调用时，AI 会直接进入对齐。
- 普通输入如果存在可能影响结果的重要未决信息，AI 可以先给出一次简短、可拒绝的建议。
...
如果用户没有明确调用，AI 只会在“继续直接处理可能因为关键未决信息而偏离目标”时，简短建议是否进入对齐模式。用户拒绝后可以继续正常对话，AI 不会反复要求进入。
