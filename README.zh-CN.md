...
`Align Before Action` 是一种先把想法聊清楚、再开始做的对齐方式。它适合想法还在形成中、上下文不完整，或者你不想让 AI 先猜再做的场景。用户可以直接调用它；当普通输入里还有重要未决信息时，AI 也可以先用一句话建议是否进入这个 Skill。进入后，它会先弄清楚你到底想做什么，再一步一步补齐关键缺口，等双方理解一致后才执行。
