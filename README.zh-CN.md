...
`Align Before Action` 是一个适合“过早执行比多讨论一轮更危险”场景的 Codex Skill。用户可以直接调用它；当普通输入中存在可能影响结果的重要未决信息时，AI 也可以先用一句话建议是否进入这个 Skill。进入后，它会先完整理解用户的想法，再一次完善一个重要缺口，形成双方一致的最终确认稿，最后才在用户明确授权后执行。
...
Skill 会在内部维护一份精简的“对齐状态”，优先询问当前能够回答、且最影响结果的问题。在引入 AI 提出的方案或取舍前，它必须单独给出一次整体理解稿；此前的局部回答不能静默解锁完善阶段。用户不知道怎么回答时，AI 会逐级提供帮助：先直接提问，再给覆盖关键方向的选项，然后给出暂定说法，最后才在必要时给出有依据的建议。选项通常为 3～5 个实质不同的方向；更多方向会先分组，再逐步缩小。完善阶段也只采用当前最相关的一个视角，不会一次抛出一长串问题。

用户可以明确停止对齐并指定立即执行的动作。此时 AI 会说明最重要的未解决风险一次，然后退出对齐并交接执行，不会把实际交付物包装成另一份“待确认稿”。
