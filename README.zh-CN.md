...
`Align Before Action` 是一个用于行动前澄清的 Skill，适合 AI 可能因为意图不完整或有歧义而误解后直接行动的场景。

它会通过聚焦的对话，先理解你真正想表达的内容，找出会影响结果的重要缺口，并帮助你完善想法，然后再进入方案、其他 Skill 或外部行动。

它适用于想法构思、决策、写作、规划、请求，以及任何不同理解可能导致不同结果的日常场景。

这个 Skill 不替你做决定。你的意图尚未确认前，AI 的假设和建议都会保持为暂定内容；理解一致后，AI 会推荐最合适的下一步。修改文件、对外沟通、发布或部署等行动，仍需你的明确授权。
